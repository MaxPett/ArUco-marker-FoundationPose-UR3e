...
try:
    ...
    while True:
        ...
        if RecordStream:
            frame_name = int(round(time.time() * 1000))
            # Define the path to the image file within the subfolder
            image_path_depth = os.path.join(subfolder_depth, f'{frame_name}.png')
            image_path_depth_unaligned = os.path.join(subfolder_depth_unaligned, f'{frame_name}.png')
            image_path_rgb_unaligned = os.path.join(subfolder_rgb_unaligned, f'{frame_name}.png')

            # for naming in [f"{frame_name}.png", f"{frame_count:07d}.jpg"]:
            for naming in [f"{frame_name}.png", f"{frame_name}.jpg"]:
                image_path_rgb = os.path.join(subfolder_rgb, naming)
